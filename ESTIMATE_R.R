...
###################################################################################################
######################PRIMARY SAMPLES ANALYSIS STAGE 4 VS STAGE 3 ####################################################################
##############################################################################################
in.file <- "C:/Users/ritas/Desktop/TESE_IMM/3_lung_deconvolution/unique_bulk_data_jan_PRIMARY.txt"
out.file <- tempfile(pattern="estimate", fileext=".gct")
outputGCT(in.file, out.file)
...
out_2.file <- tempfile(pattern="estimate", fileext=".gct")
estimateScore(out.file, out_2.file,platform = "illumina")
LC_estimate<-fread("C:\\Users\\ritas\\AppData\\Local\\Temp\\RtmpwXAdz9\\estimate43f446de57ee.gct")
View(LC_estimate)
...
#NOTE:IN THE CASE OF PRIMARY SAMPLES
metadata<-metadata[metadata$TYPE=="PRIMARIO",]
metadata<-metadata[complete.cases(metadata$STAGE),]
View(metadata)
...
#NOTE PRIMARY NP AND P
df1_filtered<-as.data.frame(df1_filtered)
df1_filtered$STAGE<-c("3","4","4","3","4","4","3","4","3","4","4","4","4","4","3","4","4","4","4","4","3","3","3","4","3","4","4","4","4","3","4")
...
#Step 1:CALCULATE THE SIGNIFICANCE LEVEL OF THE DIFFERENCE BETWEEN THE 2 GROUPS OF PATIENTS REGARDING THE TOTAL B CELLS FRACTIONS
test_result_Stromal<- t.test(df1_filtered$StromalScore ~ df1_filtered$STAGE, data = df1_filtered)

group_colors <- c("turquoise4", "orange2")

# Plot ESTIMATE scores per patient using a boxplot or violin plot
STROMAL_plot_RESPONSE<-ggplot(df1_filtered, aes(x = df1_filtered$STAGE, y = df1_filtered$StromalScore,fill=df1_filtered$STAGE)) +
  geom_boxplot(show.legend = FALSE) +  # Boxplot
  # geom_violin() +  # Violin plot (you can choose one of these)
  labs(x="",y = "Stromal Score") +
  scale_fill_manual(values = group_colors) +
  geom_signif(comparisons = list(c("3", "4")), map_signif_level = TRUE, textsize = 4) +
  geom_text(aes(label = paste("p =", signif(test_result_Stromal$p.value, digits = 2))), x = 1.5, y = max(df1_filtered$StromalScore), vjust = -0.5) +
  theme_minimal()
...
#Step 1:CALCULATE THE SIGNIFICANCE LEVEL OF THE DIFFERENCE BETWEEN THE 2 GROUPS OF PATIENTS REGARDING THE TOTAL B CELLS FRACTIONS
test_result_Immune<- t.test(df1_filtered$ImmuneScore ~ df1_filtered$STAGE, data = df1_filtered)

group_colors <- c("turquoise4", "orange2")

# Plot ESTIMATE scores per patient using a boxplot or violin plot
Immune_plot_RESPONSE<-ggplot(df1_filtered, aes(x = df1_filtered$STAGE, y = df1_filtered$ImmuneScore,fill=df1_filtered$STAGE)) +
  geom_boxplot(show.legend = FALSE) +  # Boxplot
  # geom_violin() +  # Violin plot (you can choose one of these)
  labs(x="",y = "Immune Score") +
  scale_fill_manual(values = group_colors) +
...
#Step 1:CALCULATE THE SIGNIFICANCE LEVEL OF THE DIFFERENCE BETWEEN THE 2 GROUPS OF PATIENTS REGARDING THE TOTAL B CELLS FRACTIONS
test_result_ESTIMATE<- t.test(df1_filtered$ESTIMATEScore ~ df1_filtered$STAGE, data = df1_filtered)

group_colors <- c("turquoise4", "orange2")

# Plot ESTIMATE scores per patient using a boxplot or violin plot
ESTIMATE_plot_RESPONSE<-ggplot(df1_filtered, aes(x = df1_filtered$STAGE, y = df1_filtered$ESTIMATEScore,fill=df1_filtered$STAGE)) +
  geom_boxplot(show.legend = FALSE) +  # Boxplot
  # geom_violin() +  # Violin plot (you can choose one of these)
  labs(x="",y = "ESTIMATE Score") +
  scale_fill_manual(values = group_colors) +
